...
export async function GET() {
  try {
    ...
    return handleApiError(error, "tracker")
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const rl = await rateLimitPersistent(`tracker:put:${authRateLimitKey(request)}`, {
      limit: 10,
      windowMs: 60_000,
      failClosed: false,
    })
    ...
    if (profileError) {
      return handleApiError(profileError, "tracker.update")
    }
